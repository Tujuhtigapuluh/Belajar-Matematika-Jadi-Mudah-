...
/** Format hasil pembagian dengan rapi */
const formatDiv = (dividend: number, divisor: number): string => {
  if (divisor === 0) return '∞';
  const result = dividend / divisor;
  if (Number.isInteger(result)) return result.toString();
  // Tampilkan sebagai desimal pendek atau pecahan
  const rounded = Math.round(result * 100) / 100;
  return rounded.toString();
};

/** Cek apakah habis dibagi */
      ...
      {view === 'table' ? (
        <>
          ...
          <div className="overflow-x-auto -mx-4 px-4">
            <table className="text-center border-collapse" style={{ minWidth: maxNum > 10 ? '600px' : 'auto', width: '100%' }}>
              <thead>
                <tr>
              ...
              <tbody>
                {numbers.map((row) => (
                  <tr key={row}>
                    {/* Row header (bilangan yang dibagi) */}
                    <td
                      data-row={row}
          ...
          {/* Quick Reference - Pola pembagian */}
        ...
        <>
          ...
          {/* Preset angka populer */}
          <div className="space-y-2">
            <span className="text-xs text-gray-500">Coba angka:</span>
            <div className="grid grid-cols-5 gap-2">
              {[10, 12, 20, 24, 25, 36, 48, 50, 60, 72, 100, 120, 144, 360, 1000].map((x) => (
                <button
                  key={x}
          ...
          {customValid && (
            <div className="bg-indigo-500/10 rounded-xl p-4 border border-indigo-500/20">
              <h4 className="text-sm font-medium text-indigo-300 mb-2">📊 Ringkasan {customValue}</h4>
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div className="text-gray-400">
                  Habis dibagi:{' '}
                  <span className="text-green-400 font-medium">
                    {NUMBERS_1_TO_10.filter((d) => isDivisible(customValue, d)).join(', ') || '—'}
                  </span>
                </div>
                <div className="text-gray-400">
                  Tidak habis:{' '}
