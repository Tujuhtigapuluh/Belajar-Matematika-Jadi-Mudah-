...
export const MathInput: React.FC<MathInputProps> = ({
  ...
  // Convert input to display LaTeX
  ...
  const handleModalSubmit = useCallback(() => {
    if (modal.type === 'fraction' && modal.data.numerator && modal.data.denominator) {
      // Validate: denominator cannot be 0
      ...
      // Validate: radicand should be non-negative for real numbers
...
function sanitizeInput(input: string): string {
  // Remove dangerous characters, keep math-safe ones
  return input
...
function sanitizeExpression(input: string): string {
  // Allow numbers, letters, basic operators
...
function convertToDisplayLatex(input: string): string {
  ...
  // Convert standalone fractions: a/b -> frac{a}{b} (simple cases)
  latex = latex.replace(/(\d+)\/(\d+)/g, 'frac{$1}{$2}');
  
  // Convert powers: a^b -> a sup{b}
  ...
  // Convert division symbol (standalone)
  latex = latex.replace(/÷/g, '÷');
