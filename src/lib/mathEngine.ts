...
export class Parser {
  ...
  private parseMultiplicative(): ASTNode {
    ...
    // Implicit multiplication: 2x, xy, 2(3+4)
    while (
      (this.current().type === 'NUMBER' || 
       this.current().type === 'VARIABLE' || 
      ...
      if (prevToken?.type === 'NUMBER' || prevToken?.type === 'VARIABLE' || prevToken?.type === 'RPAREN') {
        const right = this.parsePower();
        left = { type: 'BinaryOp', operator: '*', left, right };
      } else {
        break;
    ...
    switch (node.type) {
      ...
      case 'BinaryOp': {
        ...
        switch (node.operator) {
          ...
          case '*': return left * right;
          case '/': 
            if (right === 0) throw new Error('Pembagian dengan nol tidak terdefinisi');
            return left / right;
        ...
        switch (node.operator) {
          ...
          case '*': {
            ...
            if (leftIsNum && rightIsVar) {
              return `${left}${right}`; // 2x instead of 2 × x
            }
            return `${left} × ${right}`;
          }
...
export class SymbolicSolver {
  ...
  solve(input: string): MathResult {
    try {
      ...
      return this.errorResult(input, (error as Error).message);
    }
  }
  ...
  private getPolynomialDegree(node: ASTNode, variable: string): number {
    // Extract coefficients and find highest power
